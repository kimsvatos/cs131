...
let rec matchRuleTerm ruleSymbol ruleFunc acceptor frag deriv = 
	...
	| _ -> match frag with 
			...
			| (fragHead)::(fragTail) -> match ruleSymbol with 
						...
						| (N nonTermHead)::(nonTermTail) -> 
							matchRuleList ruleSymbol ruleFunc (matchRuleTerm nonTermTail ruleFunc acceptor frag deriv) frag deriv
